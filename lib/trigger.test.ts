...
import { createDesignRunToken, triggerDesignAgent } from "./trigger"

describe("triggerDesignAgent / createDesignRunToken", () => {
  ...
  describe("createDesignRunToken", () => {
    it("throws a handled error when TRIGGER_SECRET_KEY is not set", async () => {
      delete process.env.TRIGGER_SECRET_KEY

      await expect(createDesignRunToken("run_123")).rejects.toThrow(/TRIGGER_SECRET_KEY/)
    ...
    it("issues a public token scoped to only the given run, with a 1-hour expiration", async () => {
      ...
      const token = await createDesignRunToken("run_123")
    ...
    it("propagates a genuine upstream token-creation failure rather than swallowing it", async () => {
      ...
      await expect(createDesignRunToken("run_123")).rejects.toThrow("trigger.dev down")
    })
  })
})
