...
const { getProjectAccessMock, prismaMock, fetchSpecMarkdownMock } = vi.hoisted(() => ({
...
function getRequest() {
  return new NextRequest("http://localhost/api/projects/p1/specs/spec_1/download")
}
...
describe("GET /api/projects/[projectId]/specs/[specId]/download", () => {
  ...
  it("returns 404 when the spec exists but belongs to a different project", async () => {
    ...
    prismaMock.projectSpec.findUnique.mockResolvedValue({
      ...
      filePath: "https://blob.example/specs/some-other-project/spec_1.md",
    })
  ...
  it("returns 404 when the referenced Blob content is missing", async () => {
    ...
    prismaMock.projectSpec.findUnique.mockResolvedValue({
      projectId: "p1",
      filePath: "https://blob.example/specs/p1/spec_1.md",
    })
  ...
  it("returns 500 when fetching from Blob throws a genuine upstream error", async () => {
    ...
    fetchSpecMarkdownMock.mockRejectedValue(new Error("network down"))
  ...
  it("returns the markdown content as a downloadable attachment for an owner or collaborator, never the raw Blob URL", async () => {
    ...
    fetchSpecMarkdownMock.mockResolvedValue("# Spec\n\nGenerated content.")
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'attachment; filename="spec-spec_1.md"',
    )
    const body = await response.text()
    expect(body).toBe("# Spec\n\nGenerated content.")
    expect(body).not.toContain("blob.example")
    expect(fetchSpecMarkdownMock).toHaveBeenCalledWith("https://blob.example/specs/p1/spec_1.md")
  })
})
