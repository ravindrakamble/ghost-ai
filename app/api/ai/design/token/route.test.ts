...
const { getCallerIdentityMock, prismaMock, createDesignRunTokenMock } = vi.hoisted(() => ({
  ...
  createDesignRunTokenMock: vi.fn(),
}))
...
vi.mock("@/lib/trigger", () => ({
  createDesignRunToken: createDesignRunTokenMock,
}))
...
describe("POST /api/ai/design/token", () => {
  it("returns 401 when unauthenticated, before touching the body, Prisma, or Trigger.dev", async () => {
    ...
    expect(createDesignRunTokenMock).not.toHaveBeenCalled()
  })
  ...
  it("issues a run-scoped token for the run's owner", async () => {
    ...
    createDesignRunTokenMock.mockResolvedValue("pk_run_scoped_token")
    ...
    expect(createDesignRunTokenMock).toHaveBeenCalledWith("run_123")
  })
  ...
  it("returns 502 when issuing the token fails upstream", async () => {
    ...
    createDesignRunTokenMock.mockRejectedValue(new Error("trigger.dev down"))
