...
- **Tabs**: a shadcn `Tabs` control with exactly two triggers, "AI Architect" and "Specs". Active-vs-inactive styling is computed from local `activeTab` state (not a `data-active` attribute selector) and reuses the same `bg-accent-dim`/`text-brand` "active" pairing `ProjectSidebar`/`ShareDialog` already establish elsewhere in this app, rather than shadcn `Tabs`'s own near-invisible default `bg-accent`/`text-accent` (which resolves to `#1e1e23`/`#f0f0f4`, effectively identical to `bg-subtle` in this palette).
