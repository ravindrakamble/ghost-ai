...
**Modal** (`components/editor/starter-templates-modal.tsx`): a dialog built on the same `components/ui/dialog.tsx` primitives `share-dialog.tsx` already uses (`Dialog`/`DialogContent`/`DialogHeader`/`DialogTitle`/`DialogDescription`) — `sm:max-w-7xl rounded-3xl` (wider than `ShareDialog`'s `max-w-md`, wide enough to fit all `CANVAS_TEMPLATES` entries in one row without crowding). Note the `sm:` prefix is required, not cosmetic: `DialogContent`'s own base classes include `sm:max-w-sm`, and `tailwind-merge` only dedupes utilities that share the same variant prefix — an unprefixed `max-w-7xl` override leaves `sm:max-w-sm` in the class list, which then wins the cascade at ≥640px viewports (this silently broke two earlier width attempts on this spec before being traced to this). `CANVAS_TEMPLATES` renders as a `grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6` inside a `max-h-[70vh] overflow-y-auto` scroll region, same scrollable-region convention as `ShareDialog`'s collaborator list. Each card (`rounded-2xl`/`border-surface-border`/`bg-elevated`) shows the template's name, description, a `StarterTemplatePreview`, and a full-width Import button. Clicking Import calls `onImport(template)` then closes the dialog.
