...
- **Export as image** (a direct user request, not a numbered spec): a third button in the same pill, after another `bg-surface-border` divider. Downloads the current canvas as a PNG (`ghost-ai-canvas-{projectId}.png`) via `getNodesBounds`/`getViewportForBounds` (`@xyflow/react`) plus `toPng` (`html-to-image`) against the real `.react-flow__viewport` DOM node — the library's own documented export pattern, exporting at the diagram's authored 1:1 scale (not whatever zoom the viewer happens to be at) plus a fixed 48px margin. Disabled when the canvas has no nodes; shows a spinning `Loader2` (same busy-icon convention as `SpecsTab`'s "Generate Spec" button) while an export triggered from this button is in flight.
