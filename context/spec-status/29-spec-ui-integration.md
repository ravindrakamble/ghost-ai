...
- The raw spec text (`context/feature-specs/29-spec-ui-integration.md`) is unambiguous on its own literal scope: its three numbered "Implementation" items (spec list, preview modal, download action) and its four-item "Check When Done" checklist never mention the Generate button or triggering a new run — only viewing/previewing/downloading specs that already exist. Dev/QA following that literal text is the correct default, not a corner cut.
- I independently checked the Analyst Brief's specific textual claim that `architecture-context.md`'s Realtime Conventions section says spec 27's run-metadata mechanism exists "so a future spec wiring up the frontend (spec 29) doesn't have to re-derive this." Reading the actual file (and `git show` on the commit that introduced that bullet, spec 27's `9ad547d`), that exact phrase does not appear — the real text only explains why spec 27 chose Trigger.dev run-metadata over a Liveblocks broadcast ("`generate-spec` has no frontend wiring yet for a Liveblocks broadcast to reach anyway"), a technical justification for a spec-27-scoped decision, not a forward commitment obligating spec 29 to wire the button. This is a minor citation-accuracy issue in the Analyst Brief's Open Questions #1, not a substantive one — if anything, the real text is neutral-to-supportive of leaving the wiring for a later, not-yet-defined spec, so it does not change the scope call itself. Noting it here for pipeline-hygiene awareness; not routing back for it, since it did not change the brief's own recommendation or Dev's resulting implementation.
...
- **Recommendation to the human**: define a follow-up feature spec (spec 30 or later) that wires the "Generate Spec" button to `POST /api/ai/spec` + `POST /api/ai/spec/token` + `useRealtimeRun`, mirroring `ai-architect-tab.tsx`'s (spec 26) already-proven two-call submission pattern, so the full generate -> persist -> view -> download loop becomes reachable from the UI end to end. Until that lands, this pipeline's "AI Architecture Generation" and "Spec Generation" feature areas are asymmetric: architecture generation is fully UI-triggerable (spec 26), spec generation is not.
